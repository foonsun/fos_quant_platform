...
def main():
    start_date = datetime.date(2019,4,16)
    dates =[]
    for i in range(1):
        date = start_date + datetime.timedelta(i)
        date_str = date.strftime('%Y-%m-%d')
        dates.append(date_str)
    #contracts = ['binance/btc.usdt','bitfinex/btc.usd','kraken/btc.usd','bitstamp/btc.usd']
    contracts = ['huobip/new.ht']
    for date in dates:
        for contract in contracts:
            #date = '2019-04-02'
            #contract = 'binance/btc.usdt'
            #contract = 'bitfinex/btc.usd'
            #contract = 'kraken/btc.usd'
            #contract = 'bitstamp/btc.usd'
            #simple tick
            ...
            #unzip_and_read('tick-full-2019-04-02-binance-btc.usdt.gz', 0.0001)
